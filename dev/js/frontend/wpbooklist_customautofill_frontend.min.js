...
jQuery( document ).ready( function( $ ) {
	...
	// Function to change the color of the Selects on change
	wpbooklistCustomAutofillChangeSelectColor();

	// Function to display tooltips on the 'Add Simple Product' page when a questionmark icon is hovered over.
	wpbooklistCustomAutofillAddProductTooltips();

	// Function that prevents the addition of too many items in a cart.
	wpbooklistCustomAutoFillAddToCartCheck();

	/* ENDING SECTION TO CALL ALL FUNCTIONS IN FILE... */


	// Function that prevents the addition of too many items in a cart.
	function wpbooklistCustomAutoFillAddToCartCheck() {

		if ( -1 < window.location.href.indexOf( '/shop/' ) ) {
	...
	// Function to display tooltips on the 'Add Simple Product' page when a questionmark icon is hovered over.
	function wpbooklistCustomAutofillAddProductTooltips(){

		if ( -1 < window.location.href.indexOf( 'product/simple/edit/' ) ) {

			// Closing the modal by clicking anywhere on the top modal wrapper div...
			$( document ).on( 'click', '.jre-custom-tooltip-modal', function( event ) {
				...
				$( this ).animate({
					...
					duration: 500,
					complete: function() {
						$( '.jre-custom-tooltip-modal' ).remove();
					}
				});

			});

			$( 'body' ).on( 'mouseenter', '.jre-custom-question-img-tooltip', function() {
			...
			$( 'body' ).on( 'mouseenter', '#jre-custom-tooltip-wrapper-regprice img', function() {
	...
	function customSearchSidebarPopulate(){
		...
		if ( -1 < window.location.href.indexOf( 'wrls_search_in' ) ) {
			...
			// Now handle the changing of the 'Search Results' message to zero, if there were no actual results found with all of our Search Refinements.
			if ( '0' === $( '#jre-customsearch-results-indicator' ).attr( 'data-results' ) ) {
				$( '.woocommerce-result-count' ).text( 'No products were found matching your selection.' );
			} else {

				// Reveal the pagination area, if there are more than 0 results.
				$( '.post-type-archive-product .woocommerce-pagination' ).css({'opacity':'1', 'pointer-events':'all'})
			}

			// Reveal the search results text, whatever it may be at this point.
			$( '.woocommerce-result-count' ).css({'opacity':'1'})
	...
	// Function to change the color of the Selects on change
	function wpbooklistCustomAutofillChangeSelectColor(){

		if ( -1 < window.location.href.indexOf( 'hoot-vendor-dashboard/product/simple/edit' ) ) {

			$( document ).on( 'change', '#wcv_custom_product_subject, #wcv_custom_product_age, #wcv_custom_product_condition, #wcv_custom_product_grade', function( event ) {
				$( this ).css({'color':'#666'})
			});
